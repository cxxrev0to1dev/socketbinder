...
#define wait_event_interruptible(x,y) pthread_cond_wait(x,y)
//pthread_cond_signal pthread_cond_broadcast
#define wake_up_interruptible(x) pthread_cond_signal(x)
#define init_waitqueue_head(x) pthread_cond_init(x, NULL)
#define init_mutex_head(x) pthread_mutex_init(x, NULL)
#endif
...
// TODO: 用户空间跟内核空间建立映射
	...
	if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		WinPrintf( "binder: %d: %s pages %p-%p\n",
	...
	if (vma == NULL) {
		WinPrintf( "binder: %d: binder_alloc_buf failed to "
	...
	for (page_addr = start; page_addr < end; page_addr += PAGE_SIZE) {
		...
		*page = alloc_page(GFP_KERNEL | __GFP_ZERO);
		if (*page == NULL) {
			WinPrintf( "binder: %d: binder_alloc_buf failed "
			       "for page at %p\n", proc->pid, page_addr);
...
// remove priority 任务结构体相关 
	...
	if (binder_debug_mask & BINDER_DEBUG_PRIORITY_CAP)
		WinPrintf( "binder: %d: nice value %ld not allowed use "
...
{
	mutex_trylock(&proc->buffer_operation_lock);
	...
	mutex_unlock(&proc->buffer_operation_lock);
}
	...
	struct binder_proc *proc, struct binder_buffer *new_buffer)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		WinPrintf("binder: %d: add free buffer, size %zd, "
	...
	struct binder_proc *proc, void __user *user_ptr)
{
	...
	kern_ptr = (struct binder_buffer*)((ptrdiff_t)user_ptr - proc->user_buffer_offset
	...
	struct binder_proc *proc, struct binder_buffer *buffer)
{
	...
	if (buffer_end_page(prev) == buffer_start_page(buffer)) {
		...
		if (buffer_end_page(prev) == buffer_end_page(buffer))
			free_page_end = 0;
		if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
			WinPrintf("binder: %d: merge free, buffer %p "
	...
	if (free_page_start || free_page_end) {
		if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
			WinPrintf("binder: %d: merge free, buffer %p do "
	...
	size_t data_size, size_t offsets_size, int is_async)
{
...
#if 0
	if (proc->vma == NULL) {
		WinPrintf( "binder: %d: binder_alloc_buf, no vma\n",
		       proc->pid);
		return NULL;
	}
#endif
	...
	if (is_async &&
		...
		if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
			WinPrintf( "binder: %d: binder_alloc_buf size %zd f"
	...
	if (best_fit == NULL) {
		WinPrintf( "binder: %d: binder_alloc_buf size %zd failed, "
		       "no address space\n", proc->pid, size);
		return NULL;
	}
	if (n == NULL) {
	...
	if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		WinPrintf( "binder: %d: binder_alloc_buf size %zd got buff"
	...
	if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		WinPrintf( "binder: %d: binder_alloc_buf size %zd got "
	...
	if (is_async) {
		...
		if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC_ASYNC)
			WinPrintf( "binder: %d: binder_alloc_buf size %zd "
			       "async free %zd\n", proc->pid, size,
		...
		if (hlist_empty(&node->refs) && !node->local_strong_refs &&
			...
			if (node->proc) {
				...
				if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
					WinPrintf("binder: refless node %d deleted\n", node->debug_id);
			}
			else {
				hlist_del(&node->dead_node);
				if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
					WinPrintf("binder: dead node %d deleted\n", node->debug_id);
			}
			kfree(node);
...
// 	e->debug_id = tttttt->debug_id;

	if (binder_debug_mask & BINDER_DEBUG_TRANSACTION) {
		if (reply)
			WinPrintf( "binder: %d:%d BC_REPLY %d -> %d:%d, "
			       "data %p-%p size %zd-%zd\n",
			...
			WinPrintf( "binder: %d:%d BC_TRANSACTION %d -> "
...
// 		    !IS_ALIGNED(*offp, size_t, sizeof(void *))) {
		...
		case BINDER_TYPE_WEAK_BINDER: {
			...
			if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
				WinPrintf( "        node %d u%p -> ref %d desc %d\n",
		...
		case BINDER_TYPE_WEAK_HANDLE: {
			...
			if (ref->node->proc == target_proc) {
				...
				if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
					WinPrintf( "        ref %d desc %d -> node %d u%p\n",
				...
				if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
					WinPrintf( "        ref %d desc %d -> ref %d desc %d (node %d)\n",
...
err_no_context_mgr_node:
	if (binder_debug_mask & BINDER_DEBUG_FAILED_TRANSACTION)
		WinPrintf( "binder: %d:%d transaction failed %d, size"
				"%zd-%zd\n",
	...
	struct binder_proc *proc, struct binder_buffer *buffer)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		WinPrintf( "binder: %d: binder_free_buf %p size %zd buffer"
	...
	if (buffer->async_transaction) {
		proc->free_async_space += size + sizeof(struct binder_buffer);
		if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC_ASYNC)
			WinPrintf( "binder: %d: binder_free_buf size %zd "
			       "async free %zd\n", proc->pid, size,
			       proc->free_async_space);
	}
...
binder_transaction_buffer_release(struct binder_proc *proc, struct binder_buffer *buffer, size_t *failed_at)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
		WinPrintf( "binder: %d buffer release %d, size %zd-%zd, failed at %p\n",
...
// 		    !IS_ALIGNED(*offp, size_t, sizeof(void *))) {
			WinPrintf( "binder: transaction release %d bad"
		...
		case BINDER_TYPE_WEAK_BINDER: {
			struct binder_node *node = binder_get_node(proc, fp->binder);
			if (node == NULL) {
				WinPrintf( "binder: transaction release %d bad node %p\n", debug_id, fp->binder);
				break;
			}
			if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
				WinPrintf( "        node %d u%p\n",
		...
		case BINDER_TYPE_WEAK_HANDLE: {
			struct binder_ref *ref = binder_get_ref(proc, fp->handle);
			if (ref == NULL) {
				WinPrintf( "binder: transaction release %d bad handle %ld\n", debug_id, fp->handle);
				break;
			}
			if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
				WinPrintf( "        ref %d desc %d (node %d)\n",
		...
		default:
			WinPrintf( "binder: transaction release %d bad object type %lx\n", debug_id, fp->type);
			break;
		}
	}
}
	...
	if (strong) {
		if (internal) {
			if (target_list == NULL &&
			    ...
			    node->has_strong_ref)) {
				WinPrintf( "binder: invalid inc strong "
					"node for %d\n", node->debug_id);
				return -EINVAL;
			}
			node->internal_strong_refs++;
		...
		if (!node->has_weak_ref && list_empty(&node->work.entry)) {
			if (target_list == NULL) {
				WinPrintf( "binder: invalid inc weak node "
					"for %d\n", node->debug_id);
				return -EINVAL;
			}
			list_add_tail(&node->work.entry, target_list);
...
{
	if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
		WinPrintf( "binder: %d delete ref %d desc %d for "
	...
	if (ref->death) {
		if (binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
			WinPrintf( "binder: %d delete ref %d desc %d "
...
binder_get_ref_for_node(struct binder_proc *proc, struct binder_node *node)
{
	...
	if (node) {
		...
		if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
			WinPrintf( "binder: %d new ref %d desc %d for "
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		case BC_DECREFS: {
			...
			if (binder_debug_mask & BINDER_DEBUG_USER_REFS)
				WinPrintf( "binder: %d:%d %s ref %d desc %d s %d w %d for node %d\n",
		...
		case BC_ACQUIRE_DONE: {
			...
			if (binder_debug_mask & BINDER_DEBUG_USER_REFS)
				WinPrintf( "binder: %d:%d %s node %d ls %d lw %d\n",
		...
		case BC_ATTEMPT_ACQUIRE:
			WinPrintf( "binder: BC_ATTEMPT_ACQUIRE not supported\n");
			return -EINVAL;
		case BC_ACQUIRE_RESULT:
			WinPrintf( "binder: BC_ACQUIRE_RESULT not supported\n");
			return -EINVAL;
		...
		case BC_FREE_BUFFER: {
			...
			if (binder_debug_mask & BINDER_DEBUG_FREE_BUFFER)
				WinPrintf( "binder: %d:%d BC_FREE_BUFFER u%p found buffer %d for %s transaction\n",
		...
		case BC_REGISTER_LOOPER:
			...
			if (binder_debug_mask & BINDER_DEBUG_THREADS)
				WinPrintf( "binder: %d:%d BC_REGISTER_LOOPER\n",
				       proc->pid, thread->pid);
		...
		case BC_ENTER_LOOPER:
			//CHECK OK
			if (binder_debug_mask & BINDER_DEBUG_THREADS)
				WinPrintf( "binder: %d:%d BC_ENTER_LOOPER\n",
				       proc->pid, thread->pid);
		...
		case BC_EXIT_LOOPER:
			//CHECK OK
			if (binder_debug_mask & BINDER_DEBUG_THREADS)
				WinPrintf( "binder: %d:%d BC_EXIT_LOOPER\n",
				       proc->pid, thread->pid);
		...
		case BC_CLEAR_DEATH_NOTIFICATION: {
			...
			if (binder_debug_mask & BINDER_DEBUG_DEATH_NOTIFICATION)
				WinPrintf( "binder: %d:%d %s %p ref %d desc %d s %d w %d for node %d\n",
				       proc->pid, thread->pid,
				       cmd == BC_REQUEST_DEATH_NOTIFICATION ?
				       "BC_REQUEST_DEATH_NOTIFICATION" :
				       "BC_CLEAR_DEATH_NOTIFICATION",
	 ...
	 if (list_empty(&death->work.entry)) {
		...
		case BC_DEAD_BINDER_DONE: {
			...
			if (binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
				WinPrintf("binder: %d:%d BC_DEAD_BINDER_DONE %p found %p\n",
		...
		default:
			WinPrintf("binder: %d:%d unknown command %d\n", proc->pid, thread->pid, cmd);
			return -EINVAL;
		}
	...
	if (wait_for_proc_work) {
		...
		if (non_block) {
			...
			while (!binder_has_proc_work(proc, thread)) {
				...
				// 					pthread_cond_timedwait(&proc->wait, &binder_lock, &timeToWait);
				// 					continue;
				// 				}
				wait_event_interruptible((pthread_cond_t*)&proc->wait, &binder_lock);
			}
			{
				mutex_unlock(&binder_lock);
			}
		...
		if (non_block) {
			...
			while (!binder_has_thread_work(thread)) {
				wait_event_interruptible(&thread->wait, &binder_lock);
			}
			{
				mutex_unlock(&binder_lock);
			}
	...
	while (1) {
		...
		struct binder_transaction *t = NULL;
		{
			if (!list_empty(&thread->todo))
				...
				if ((((unsigned long)ptr - (unsigned long)buffer) == 4) && !(thread->looper & BINDER_LOOPER_STATE_NEED_RETURN)) /* no data added */
					goto retry;
				break;
			}
		}
		...
		case binder_work::BINDER_WORK_TRANSACTION_COMPLETE: {
			...
			if (binder_debug_mask & BINDER_DEBUG_TRANSACTION_COMPLETE)
				WinPrintf( "binder: %d:%d BR_TRANSACTION_COMPLETE\n",
				       proc->pid, thread->pid);
		...
		case binder_work::BINDER_WORK_NODE: {
			...
			if (cmd != BR_NOOP) {
				...
				binder_stat_br(proc, thread, cmd);
				if (binder_debug_mask & BINDER_DEBUG_USER_REFS)
					WinPrintf( "binder: %d:%d %s %d u%p c%p\n",
					       proc->pid, thread->pid, cmd_name, node->debug_id, node->ptr, node->cookie);
			} else {
				list_del_init(&w->entry);
				if (!weak && !strong) {
					if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
						WinPrintf( "binder: %d:%d node %d u%p c%p deleted\n",
					...
					if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
						WinPrintf( "binder: %d:%d node %d u%p c%p state unchanged\n",
						       proc->pid, thread->pid, node->debug_id, node->ptr, node->cookie);
				}
			}
		} break;
...
/*		WinPrintf("tr.data.ptr.buffer %p:t->buffer->data_size %p:ALIGN %p", tr.data.ptr.buffer, t->buffer->data_size, ALIGN(t->buffer->data_size, size_t, sizeof(void *)));*/
		put_user(cmd, (uint32_t __user *)ptr);
		ptr = (void*)((uint32_t)ptr + sizeof(uint32_t));
...
binder_send_failed_reply(struct binder_transaction *t, uint32_t error_code)
{
	...
	while (1) {
		...
		if (target_thread) {
			...
			if (target_thread->return_error == BR_OK) {
				if (binder_debug_mask & BINDER_DEBUG_FAILED_TRANSACTION)
					WinPrintf( "binder: send failed reply for transaction %d to %d:%d\n",
					       t->debug_id, target_thread->proc->pid, target_thread->pid);
				binder_pop_transaction(target_thread, t);
				target_thread->return_error = error_code;
				wake_up_interruptible(&target_thread->wait);
			} else {
				WinPrintf( "binder: reply failed, target "
			...
			if (binder_debug_mask & BINDER_DEBUG_FAILED_TRANSACTION)
				WinPrintf( "binder: send failed reply "
					"for transaction %d, target dead\n",
					t->debug_id);
			binder_pop_transaction(target_thread, t);
			if (next == NULL) {
				if (binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
					WinPrintf( "binder: reply failed,"
						" no target thread at root\n");
				return;
			}
			t = next;
			if (binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
				WinPrintf( "binder: reply failed, no targ"
					"et thread -- retry %d\n", t->debug_id);
		}
	}
}
...
static int binder_free_thread(struct binder_proc *proc, struct binder_thread *thread)
{
	...
	while (t) {
		...
		if (binder_debug_mask & BINDER_DEBUG_DEAD_TRANSACTION)
			WinPrintf( "binder: release %d:%d transaction %d %s, still active\n",
...
binder_new_node(struct binder_proc *proc, void __user *ptr, void __user *cookie)
{
	...
	#if 0
	if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
		WinPrintf( "binder: %d:%d node %d u%p c%p created\n",
...
long binder_set_context_mgr(unsigned int cmd, unsigned long arg, int f_flags, int pid, int tid) {
	...
	if (binder_context_mgr_node != NULL) {
		WinPrintf("binder: BINDER_SET_CONTEXT_MGR already set\n");
		ret = -EBUSY;
		goto err;
	}
	if (binder_context_mgr_uid != -1) {
		if (binder_context_mgr_uid != proc->uid) {// current->cred->euid) {
			WinPrintf("binder: BINDER_SET_"
...
err:
	...
	//	WinPrintf( "binder: %d:%d ioctl %x %lx returned %d\n", proc->pid, current->pid, cmd, arg, ret);
...
//		WinPrintf( "binder_open: %d:%d\n", current->group_leader->pid, current->pid);
	...
	init_mutex_head(&proc->buffer_operation_lock);
...
static void binder_deferred_flush(struct binder_proc *proc)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_OPEN_CLOSE)
		WinPrintf("binder_flush: %d woke %d threads\n", proc->pid, wake_count);
}
...
// 	BUG_ON(proc->files);
	...
	if (binder_context_mgr_node && binder_context_mgr_node->proc == proc) {
		if (binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
			WinPrintf("binder_release: %d context_mgr_node gone\n", proc->pid);
	...
	while ((n = rb_first(&proc->nodes))) {
		...
		if (hlist_empty(&node->refs)) {
			...
			if (binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
				WinPrintf("binder: node %d now dead, refs %d, death %d\n", node->debug_id, incoming_refs, death);
		}
	}
	outgoing_refs = 0;
	...
	while ((n = rb_first(&proc->allocated_buffers))) {
		...
		if (t) {
			t->buffer = NULL;
			buffer->transaction = NULL;
			WinPrintf("binder: release proc %d, transaction %d, not freed\n", proc->pid, t->debug_id);
			BUG();
		}
		binder_free_buf(proc, buffer);
		buffers++;
...
// 			if (proc->pages[i]) {
// 				if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
// 					WinPrintf("binder_release: %d: page %d at %p not freed\n", proc->pid, i, (uint32_t)proc->buffer + i * PAGE_SIZE);
...
// 	}
	...
	if (binder_debug_mask & BINDER_DEBUG_OPEN_CLOSE)
		WinPrintf("binder_release: %d threads %d, nodes %d (ref %d), refs %d, active transactions %d, buffers %d, pages %d\n",
